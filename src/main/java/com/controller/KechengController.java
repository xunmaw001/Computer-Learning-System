    ...
    */
    ...
    public R save(@RequestBody KechengEntity kecheng, HttpServletRequest request){
        ...
        if(kechengEntity==null){
            ...
            caozuorizhiService.insertCaozuorizhi(String.valueOf(request.getSession().getAttribute("role")),TABLE_NAME,String.valueOf(request.getSession().getAttribute("username")),"新增",kecheng.toString());
            return R.ok();
        }else {
...
//            return R.error(511,"永远不会进入");
        ...
        if("".equals(kecheng.getKechengContent()) || "null".equals(kecheng.getKechengContent())){
            ...
            List<String> strings = caozuorizhiService.clazzDiff(kecheng, oldKechengEntity, request,new String[]{"updateTime"});
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        caozuorizhiService.insertCaozuorizhi(String.valueOf(request.getSession().getAttribute("role")),TABLE_NAME,String.valueOf(request.getSession().getAttribute("username")),"删除",oldKechengList.toString());
        return R.ok();
    }
...
//                            kechengEntity.setKechengNewMoney(data.get(0));                    //金额 要改的
//                            kechengEntity.setKechengTypes(Integer.valueOf(data.get(0)));   //课程类型 要改的
//                            kechengEntity.setKechengTime(sdf.parse(data.get(0)));          //上课时间 要改的
...
//                            kechengEntity.setCreateTime(date);//时间
                        ...
                        caozuorizhiService.insertCaozuorizhi(String.valueOf(request.getSession().getAttribute("role")),TABLE_NAME,String.valueOf(request.getSession().getAttribute("username")),"批量新增",kechengList.toString());
                        return R.ok();
                    }
                }
            }
    ...
    */
    ...
    public R gexingtuijian(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils pageUtils = kechengOrderService.queryPage(params1);
        List<KechengOrderView> orderViewsList =(List<KechengOrderView>)pageUtils.getList();
        Map<Integer,Integer> typeMap=new HashMap<>();//购买的类型list
        for(KechengOrderView orderView:orderViewsList){
            Integer kechengTypes = orderView.getKechengTypes();
...
//            .notIn("kecheng_types", new Integer[]{102})
        ...
        kechengService.insert(kecheng);
            ...
            caozuorizhiService.insertCaozuorizhi(String.valueOf(request.getSession().getAttribute("role")),TABLE_NAME,String.valueOf(request.getSession().getAttribute("username")),"前台新增",kecheng.toString());
            return R.ok();
        }else {
            return R.error(511,"表中有相同数据");
        }
